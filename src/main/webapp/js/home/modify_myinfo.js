(function($) {
	var modify_myinfo = function() {
		...
		return {
			...
			init : function() {
				student.init();
				modify_myinfo.btn_click();
				modify_myinfo.initValidate();
				modify_myinfo.show();
			},
			...
			save:function(){
				...
				vo.student_name=$.trim($("#student_name1").val());
				...
				commonAjax.ajaxPost("student/update.do", vo, function(result){
					...
					if(result.success){
						swal({title:"提示!",text: "信息修改成功!", type: "success",showCancelButton: false},function(){
	...
	window.modify_myinfo = modify_myinfo;
}(jQuery));